...
### Память для новых сессий

При начале новой сессии:
1. Проверить текущую ветку: `git branch`
2. Прочитать этот файл (`BRANCH_STRATEGY.md`)
3. Прочитать `worklog.md` для понимания прогресса
4. Если ветка или окружение изменились — **уточнить у владельца** перед изменениями
