...
После выполнения любой задачи AI-агент обязан:

1. **Записать в `worklog.md`** раздел **Рекомендации и улучшения**, даже если задача формально выполнена.

2. **Дублировать в чат** тот же блок после описания «Выполненные задачи»:
   ```
...
Самостоятельно реализовывать эти рекомендации в рамках текущей задачи нельзя, если этого прямо не попросил владелец.
