...
import { getFunnelStageLabel } from "./leak-policy-utils";

type LeakPolicyInspectorPanelProps = Omit<
...
> & {
  nextBestActionHint: LeakPolicyPanelProps["nextBestActionHint"];
};

export function LeakPolicyInspectorPanel({
  policy,
  selectedPlan,
  nextBestActionHint,
...
}: LeakPolicyInspectorPanelProps) {
  if (!(showPolicyInspector && policy)) return null;
          ...
          <div className="mt-1 flex flex-wrap gap-2">
            ...
            {currentFunnel.oldestPendingOutcomeActionId && (
              <Button
                size="sm"
                variant="outline"
                onClick={() => onFocusPlanAction(currentFunnel.oldestPendingOutcomeActionId || "")}
                  ...
                  <Button
                    ...
                    onClick={() =>
                      onSendPlanActionFeedback(
                        currentFunnel.nextPendingOutcomeActionId || "",
                        "partially",
                        onGetFeedbackCommentDraft(currentFunnel.nextPendingOutcomeActionId || "")
                      )
                    }
