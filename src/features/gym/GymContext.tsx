...
export function GymProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (activePeriod?.daySchedule) {
      try {
        const schedule =
          typeof activePeriod.daySchedule === "string"
            ? JSON.parse(activePeriod.daySchedule)
            : activePeriod.daySchedule;
        const sortedSchedule = [...schedule].sort(
          (a: DayScheduleItem, b: DayScheduleItem) => a.dayNum - b.dayNum
        );
  ...
  const getCalendarDays = useCallback((): (null | {
    date: Date;
    workout: GymWorkout | undefined;
    dayNum: number;
  })[] => {
  ...
  const handleSkipWorkout = async (shiftSchedule: boolean) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setWorkouts(
          (wData.workouts || []).map((w: GymWorkout) => ({
            ...w,
            muscleGroups: (() => {
              try {
                return typeof w.muscleGroups === "string"
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSelectedWorkout((prev) =>
          prev ? { ...prev, completed: false, status: "in_progress" } : null
        );
        loadTodayData();
      }
    } catch (error) {
    ...
    try {
      const exercisesData = selectedWorkout.exercises?.map((ex) => {
        ...
        const step =
          (
            ex.template as
              | (GymContextValue["templates"][0] & { progressionStep?: number })
              | undefined
          )?.progressionStep || 2.5;
  ...
  const loadWorkoutDetails = async (workout: GymWorkout) => {
    try {
      ...
      if (data.workout?.muscleGroups) {
        try {
          muscleGroups =
            typeof data.workout.muscleGroups === "string"
