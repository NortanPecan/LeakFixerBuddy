      ...
      {isLoading ? (
        ...
        <Card className="bg-muted/30">
          <CardContent className="py-8 text-center">
            <GraduationCap className="text-muted-foreground/50 mx-auto mb-3 h-12 w-12" />
        ...
        <div className="space-y-2">
          {items.map((item) => {
            ...
            const hasLinks =
              (item.links && item.links.length > 0) || (item.tasks && item.tasks.length > 0);
                ...
                <CardContent className="p-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {item.author && (
                        <p className="text-muted-foreground mt-1 text-xs">{item.author}</p>
                      )}
                    </div>
                    {item.url && (
                      <Button
                  ...
                  <Button
                    ...
                    onClick={() =>
                      setNewContent((prev) => ({
                        ...prev,
                        type: type.id,
                        unitType: getDefaultUnitType(type.id),
                      }))
                    }
                    className="gap-1"
                  >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label htmlFor="totalUnits">
                  Всего (
                  {newContent.unitType === "pages"
                    ? "страниц"
                    : newContent.unitType === "lessons"
                      ? "уроков"
                      : "минут"}
                  )
                </Label>
                <Input
                  ...
                  onChange={(e) =>
                    setNewContent((prev) => ({ ...prev, totalUnits: e.target.value }))
                  }
                />
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Badge variant="outline">
                  {getContentTypeInfo(selectedItem.type).icon}{" "}
                  {getContentTypeInfo(selectedItem.type).label}
                </Badge>
                <Badge variant="secondary">
                  {getContentStatusInfo(selectedItem.status).icon}{" "}
                  {getContentStatusInfo(selectedItem.status).label}
                </Badge>
                <Badge variant="outline">
                  {getNoteZoneInfo(selectedItem.zone).icon}{" "}
                  {getNoteZoneInfo(selectedItem.zone).label}
                </Badge>
              </div>

              {/* Author & Source */}
              {selectedItem.author && (
                <p className="text-muted-foreground text-sm">Автор: {selectedItem.author}</p>
              )}
              {selectedItem.source && (
                <p className="text-muted-foreground text-sm">Источник: {selectedItem.source}</p>
              )}
              ...
              <div className="bg-muted/30 rounded-lg p-3">
                <p className="mb-1 text-sm font-medium">Прогресс</p>
              ...
              {selectedItem.links && selectedItem.links.length > 0 && (
                <div className="space-y-2">
                  ...
                  {selectedItem.links.map((link) => (
                    <div key={link.id} className="bg-muted/30 rounded-lg p-2 text-sm">
