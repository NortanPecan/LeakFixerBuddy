...
export function WeeklyReportScreen() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/ai/correlations?userId=${user.id}`)
      .then((r) => r.json())
      .then((d) => {
        if (Array.isArray(d.patterns)) setAiCorrelations(d.patterns);
      })
      .catch(() => {})
  ...
  if (!report) {
    return (
      <div className="flex flex-col gap-4 pb-20">
        <p className="mt-10 text-center text-white/40">Нет данных за эту неделю</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-4 pb-20">
      ...
      <div className="grid grid-cols-4 gap-2">
        <StatBadge
          label="Настр."
          value={report.summary.avgMood}
          emoji="🎭"
          prev={prevSummary?.avgMood}
        />
        <StatBadge
          label="Энергия"
          value={report.summary.avgEnergy}
          emoji="⚡"
          prev={prevSummary?.avgEnergy}
        />
        <StatBadge
          label="Зал"
          value={report.summary.gymDays}
          emoji="💪"
          suffix="д"
          prev={prevSummary?.gymDays}
        />
        <StatBadge
          label="Чекапы"
          value={report.summary.checkinDays}
          emoji="✓"
          suffix="/7"
          prev={prevSummary?.checkinDays}
        />
      </div>
      {/* Second row of stats */}
      <div className="grid grid-cols-4 gap-2">
        <StatBadge
          label="Ритуалы"
          value={report.summary.totalRitualsCompleted}
          emoji="🔥"
          prev={prevSummary?.totalRitualsCompleted}
        />
        <StatBadge
          label="Привычки"
          value={report.summary.totalHabitsCompleted}
          emoji="🔄"
          prev={prevSummary?.totalHabitsCompleted}
        />
        <StatBadge
          label="Оценка дня"
          value={report.summary.avgEveningRating}
          emoji="🌙"
          prev={prevSummary?.avgEveningRating}
        />
        <StatBadge
          label="Расходы"
      ...
      {report.summary.bestDay &&
        (report.summary.bestDay.mood !== null || report.summary.bestDay.energy !== null) && (
          <Card
            style={{
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-muted-foreground mt-1 flex gap-3 text-xs">
                    {report.summary.bestDay.mood !== null && (
                      <span>🎭 {report.summary.bestDay.mood}/10</span>
                    )}
                    {report.summary.bestDay.energy !== null && (
                      <span>⚡ {report.summary.bestDay.energy}/10</span>
                    )}
                    {report.summary.bestDay.hadGym && <span>💪 зал</span>}
                    {report.summary.bestDay.ritualsCompleted > 0 && (
                      <span>
                        🔥 {report.summary.bestDay.ritualsCompleted}/
                        {report.summary.bestDay.ritualsTotal}
                      </span>
                    )}
                  </div>
                </div>
            ...
            {report.leakHints.map((hint, i) => (
              ...
              >
                <div className="flex gap-3">
                  <span className="flex-shrink-0 text-xl">{hint.emoji}</span>
                  <div className="flex-1">
                    <p className="text-sm text-white/80">{hint.message}</p>
                    {hint.days && (
                      <div className="mt-1 flex gap-1">
                        {hint.days.map((d) => (
                          <Badge key={d} variant="outline" className="h-4 text-[10px]">
                            {d}
                          </Badge>
                        ))}
                      </div>
      ...
      {(correlationsLoading || (aiCorrelations && aiCorrelations.length > 0)) && (
        <Card className="bg-card/50 border-violet-500/30 backdrop-blur">
          <CardHeader className="pb-2">
                ...
                {aiCorrelations!.map((p, i) => {
                  ...
                  return (
                    <div key={i} className="rounded-lg border border-white/10 p-3">
                      ...
                      <p className={`mt-1 font-mono text-xs ${strengthColor}`}>{strengthLabel}</p>
                      {p.recommendation && (
                        <p className="mt-1 text-xs text-white/60">💡 {p.recommendation}</p>
                      )}
                    </div>
                  );
                })}
...
function CorrelationInsights({ days }: { days: DayData[] }) {
  ...
  if (gymDays.length >= 2 && noGymDays.length >= 2) {
    ...
    if (avgMoodGym - avgMoodNoGym > 1.5) {
      insights.push({
        emoji: "🏋️",
        text: `В дни тренировок настроение в среднем на ${(avgMoodGym - avgMoodNoGym).toFixed(1)} балла выше`,
      });
    }
  }

  // Rituals → energy correlation
  const fullRitualDays = days.filter(
    (d) => d.ritualsTotal > 0 && d.ritualsCompleted === d.ritualsTotal && d.energy !== null
  );
  const missedRitualDays = days.filter(
    (d) => d.ritualsTotal > 0 && d.ritualsCompleted < d.ritualsTotal / 2 && d.energy !== null
  );
  if (fullRitualDays.length >= 2 && missedRitualDays.length >= 1) {
    const avgEnergyFull =
      fullRitualDays.reduce((s, d) => s + (d.energy ?? 0), 0) / fullRitualDays.length;
    const avgEnergyMissed =
      missedRitualDays.reduce((s, d) => s + (d.energy ?? 0), 0) / missedRitualDays.length;
    if (avgEnergyFull - avgEnergyMissed > 1) {
      insights.push({
        emoji: "🎯",
        text: `Когда все ритуалы выполнены — энергия выше на ${(avgEnergyFull - avgEnergyMissed).toFixed(1)} балла`,
      });
    }
  }
  ...
  const highCalThenLowEnergy = calDays
    .filter(({ d, i }) => {
  ...
  if (highCalThenLowEnergy.length >= 2) {
    insights.push({
      emoji: "🍔",
      text: `${highCalThenLowEnergy.length}× после дня переедания (>2500 ккал) энергия на следующий день падала ниже 5`,
    });
  }
  ...
  if (sleepDays.length >= 2) {
    ...
    if (goodSleepNextEnergy.length >= 2 && shortSleepNextEnergy.length >= 1) {
      ...
      const avgShort =
        shortSleepNextEnergy.reduce((a, b) => a + b, 0) / shortSleepNextEnergy.length;
      if (avgGood - avgShort > 1.5) {
        insights.push({
          emoji: "😴",
          text: `После ночи 7.5ч+ энергия на следующий день выше на ${(avgGood - avgShort).toFixed(1)} балла`,
        });
      }
    }
  }
  ...
  if (gymNextMoods.length >= 2 && noGymNextMoods.length >= 2) {
    ...
    if (avgGym - avgNo > 1.2) {
      insights.push({
        emoji: "💪",
        text: `После тренировок настроение на следующий день выше на ${(avgGym - avgNo).toFixed(1)} балла`,
      });
    }
  }
  ...
  if (withEvening.length >= 2 && withoutEvening.length >= 1) {
    const avgWith =
      withEvening.reduce((s, d) => s + (d.eveningRating ?? 0), 0) / withEvening.length;
    const avgWithout =
      withoutEvening.reduce((s, d) => s + (d.eveningRating ?? 0), 0) / withoutEvening.length;
    if (avgWith - avgWithout > 1) {
      insights.push({
        emoji: "🌙",
        text: `В дни с вечерним чекапом оценка дня выше на ${(avgWith - avgWithout).toFixed(1)} балла`,
      });
    }
  }
  ...
  return (
    ...
    >
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-base">🔗 Паттерны недели</CardTitle>
      </CardHeader>
      <CardContent className="space-y-2">
        {insights.map((ins, i) => (
...
function SmartRecommendation({ report }: { report: WeeklyReport }) {
  ...
  if (s.gymDays === 0) {
    candidates.push({
      ...
      action:
        "Ноль тренировок за неделю — запланируй хотя бы одну. Даже 30 минут изменят самочувствие и статистику.",
      score: 12,
    });
  } else if (s.gymDays === 1) {
    candidates.push({
      ...
      action:
        "Всего 1 тренировка за неделю — попробуй добавить вторую. Два раза в неделю уже системный результат.",
      score: 7,
    });
  }
  ...
  if (ritualDays.length >= 3) {
    const avgRate =
      ritualDays.reduce((acc, d) => acc + d.ritualsCompleted / d.ritualsTotal, 0) /
      ritualDays.length;
    if (avgRate < 0.5) {
      candidates.push({
        emoji: "🔥",
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-white/60">
                Неделя прошла хорошо! Выбери одну привычку и подними планку на 10%.
              </p>
            </div>
          </div>
        </CardContent>
      ...
      <CardContent className="pt-3 pb-3">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-white/60">{best.action}</p>
          </div>
        </div>
      </CardContent>
    </Card>
...
function AiPromptButton({ report }: { report: WeeklyReport }) {
  ...
  const buildPrompt = (): string => {
    ...
    lines.push(
      `Ты — эксперт по саморазвитию и поведенческому анализу. Проанализируй мою неделю (${report.weekStart} — ${report.weekEnd}) и найди закономерности, паттерны и рекомендации.`
    );
    lines.push("");
    ...
    lines.push(
      `- Средняя оценка дня: ${s.avgEveningRating > 0 ? s.avgEveningRating.toFixed(1) : "нет данных"}/10`
    );
    ...
    if (s.avgCaloriesPerDay > 0)
      lines.push(`- Среднее кКал/день: ${s.avgCaloriesPerDay.toFixed(0)}`);
    ...
    lines.push(
      "2. Объясни связи между показателями (например: низкая энергия → меньше ритуалов → хуже день)"
    );
  ...
  return (
    <button
      onClick={handleCopy}
      className="flex w-full items-center justify-center gap-2 rounded-2xl py-3 text-sm font-medium transition-all"
      style={{
...
function MoodEnergyChart({ days }: { days: DayData[] }) {
  ...
  return (
    ...
    >
      ...
      {/* Color legend */}
      <div className="mt-2 flex gap-3 text-[9px] text-white/30">
        <span>
          <span style={{ color: "#22c55e" }}>●</span> ≥7 хорошо
        </span>
        <span>
          <span style={{ color: "#f59e0b" }}>●</span> 4–6 норм
        </span>
        <span>
          <span style={{ color: "#ef4444" }}>●</span> ≤3 плохо
        </span>
      </div>
    </div>
  );
...
function DayRow({ day }: { day: DayData }) {
  ...
  return (
    <div className="flex items-center gap-3 border-b border-white/5 py-2 last:border-0">
      <div className="w-8 text-xs font-medium text-white/40">{day.dayOfWeek}</div>
      ...
      {day.ritualsTotal > 0 && (
        <div
          ...
          style={{
            color:
              day.ritualsCompleted / day.ritualsTotal >= 0.8
                ? "#22c55e"
                : day.ritualsCompleted / day.ritualsTotal >= 0.5
                  ? "#f59e0b"
                  : "#ef4444",
          }}
      ...
      {day.foodCount > 0 && (
        <div
          className="flex h-2 w-12 flex-shrink-0 overflow-hidden rounded-full"
          title={`Еда: ${day.foodGood}✅ ${day.foodNeutral}🟡 ${day.foodBad}❌ (${day.totalCalories} ккал)`}
        >
          {day.foodGood > 0 && <div style={{ flex: day.foodGood, background: "#22c55e" }} />}
          {day.foodNeutral > 0 && <div style={{ flex: day.foodNeutral, background: "#f59e0b" }} />}
          {day.foodBad > 0 && <div style={{ flex: day.foodBad, background: "#ef4444" }} />}
        </div>
      )}
