...
export function GoalsScreen() {
  ...
  const handleUpdateDirection = async () => {
    ...
    try {
      ...
      if (data.direction) {
        setDirections((prev) =>
          prev.map((d) => (d.id === editingDirection.id ? data.direction : d))
        );
        closeDirectionDialog();
      }
    } catch (err) {
  ...
  return (
    <div className="flex flex-col gap-4 pb-20">
      {/* Header */}
      <div className="flex items-center justify-between">
        <h1 className="text-foreground text-2xl font-bold">Цели</h1>
        <Button size="sm" onClick={() => setShowChallengeDialog(true)}>
          <Plus className="mr-1 h-4 w-4" /> Челендж
        </Button>
      </div>
        ...
        {directions.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="pt-4 pb-3 text-center">
              <Compass className="text-muted-foreground/50 mx-auto mb-2 h-8 w-8" />
              ...
              <Button variant="outline" size="sm" onClick={() => setShowDirectionDialog(true)}>
                <Plus className="mr-1 h-4 w-4" /> Направление
              </Button>
            </CardContent>
          </Card>
        ) : (
          <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-2">
            {directions.map((dir) => {
              ...
              const horizonLabel =
                HORIZON_OPTIONS.find((h) => h.value === dir.horizon)?.label || "Год";
              return (
                <Card
                  key={dir.id}
                  className="hover:bg-card/80 group relative w-36 flex-shrink-0 cursor-pointer transition-colors"
                  style={{ borderLeftWidth: 3, borderLeftColor: dir.color }}
                  onClick={() => openEditDirection(dir)}
                >
                  <CardContent className="px-3 pt-3 pb-2">
                    <div className="mb-1 truncate text-sm font-medium">{dir.title}</div>
                    <div className="text-muted-foreground text-xs">
                      {dirChallenges.length} активных челенджей
                    </div>
                    <div className="text-muted-foreground/70 mt-1 text-xs">🎯 {horizonLabel}</div>
                    {dir.description && (
            ...
            <Card
              className="hover:bg-muted/30 flex w-24 flex-shrink-0 cursor-pointer items-center justify-center border-dashed transition-colors"
        ...
        <CardContent className="space-y-3">
          {activeChallenges.length === 0 ? (
            <div className="py-4 text-center">
              <Target className="text-muted-foreground/50 mx-auto mb-2 h-10 w-10" />
              <p className="text-muted-foreground mb-3 text-sm">Нет активных челенджей</p>
              <Button size="sm" onClick={() => setShowChallengeDialog(true)}>
                <Plus className="mr-1 h-4 w-4" /> Создать челендж
              </Button>
            </div>
          ) : (
            activeChallenges.map((ch) => {
              ...
              return (
                <div
                  key={ch.id}
                  className="bg-muted/30 hover:bg-muted/50 flex cursor-pointer items-center gap-3 rounded-lg p-3 transition-colors"
                  onClick={() => {
                ...
                >
                  <div
                    ...
                    style={{
                      backgroundColor: ch.direction?.color
                        ? `${ch.direction.color}20`
                        : "#10b98120",
                    }}
                  >
                    {ch.type === "ritual" ? (
                      <Flame
                        className="h-5 w-5"
                        style={{ color: ch.direction?.color || "#10b981" }}
                      />
                    ) : ch.type === "chain" ? (
                      <Target
                        className="h-5 w-5"
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-muted-foreground mt-1 flex items-center gap-2 text-xs">
                      ...
                      <span>
                        {ch.daysCompleted}/{ch.duration} дней
                      </span>
                      {ch.currentStreak > 0 && (
                        <>
                          <span>•</span>
        ...
        <Card className="bg-card/50">
          ...
          <CardContent className="space-y-2">
            {plannedChallenges.map((ch) => {
              const catInfo = getCategoryInfo(ch.category);
              return (
                <div key={ch.id} className="bg-muted/20 flex items-center gap-3 rounded-lg p-3">
                  <div
                    className="hover:bg-muted/30 flex-1 cursor-pointer rounded-lg"
                    onClick={() => {
                      setSelectedContentId(ch.id);
                      setScreen("challenge-detail");
                    }}
                  >
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      <span className="text-muted-foreground text-xs">{ch.duration} дней</span>
                    </div>
                  </div>
                  <Button
                    size="sm"
        ...
        <Card className="bg-card/50">
          ...
          <CardContent className="space-y-2">
            {completedChallenges.slice(0, 5).map((ch) => (
              <div key={ch.id} className="flex items-center gap-3 rounded-lg p-2 opacity-70">
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setDirectionForm((prev) => ({ ...prev, description: e.target.value }))
                }
              ...
              <div className="flex gap-2">
                {COLORS.map((c) => (
                  <button
                    key={c}
                    type="button"
                    className={`h-8 w-8 rounded-full transition-all ${directionForm.color === c ? "ring-primary ring-2 ring-offset-2" : ""}`}
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  setChallengeForm((prev) => ({ ...prev, description: e.target.value }))
                }
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) =>
                  setChallengeForm((prev) => ({ ...prev, directionId: v === "_none" ? "" : v }))
                }
              >
                <SelectTrigger>
            ...
            <div className="space-y-3">
              ...
              {availableRituals.length > 0 && (
                <div className="space-y-2">
                  <div className="text-muted-foreground flex items-center gap-1 text-xs">
                    <Flame className="h-3 w-3" /> Ритуалы
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {availableRituals.slice(0, 6).map((r) => (
                      <Badge
                        key={r.id}
                        variant={
                          challengeForm.linkedRitualIds.includes(r.id) ? "default" : "outline"
                        }
              ...
              {availableSkills.length > 0 && (
                <div className="space-y-2">
                  <div className="text-muted-foreground flex items-center gap-1 text-xs">
                    <Sparkles className="h-3 w-3" /> Навыки
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {availableSkills.slice(0, 6).map((s) => (
                      <Badge
                        key={s.id}
                        variant={
                          challengeForm.linkedSkillIds.includes(s.id) ? "default" : "outline"
                        }
                        className="cursor-pointer text-xs"
                        onClick={() =>
                          setChallengeForm((prev) => ({
              ...
              {availableTraits.length > 0 && (
                <div className="space-y-2">
                  <div className="text-muted-foreground flex items-center gap-1 text-xs">
                    <Target className="h-3 w-3" /> Качества
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {availableTraits.slice(0, 6).map((t) => (
                      <Badge
                        key={t.id}
                        variant={
                          challengeForm.linkedTraitIds.includes(t.id) ? "default" : "outline"
                        }
                        className="cursor-pointer text-xs"
                        onClick={() =>
                          setChallengeForm((prev) => ({
