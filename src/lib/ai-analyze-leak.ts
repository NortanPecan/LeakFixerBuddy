...
export async function analyzeLeakWithAI(input: AnalyzeLeakInput): Promise<AnalyzeLeakResult> {
  ...
  const avgMood   = moodDays.length   ? moodDays.reduce((s, d)   => s + (d.mood ?? 0), 0)   / moodDays.length   : 0
  const avgEnergy = energyDays.length ? energyDays.reduce((s, c) => s + (c.energy ?? 0), 0) / energyDays.length : 0
  const avgSleep  = sleepDays.length  ? sleepDays.reduce((s, d)  => s + (d.sleepHours ?? 0), 0) / sleepDays.length : 0
