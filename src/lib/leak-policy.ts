...
export function buildContextDrift(
  ...
  const avgDelta =
    top.length > 0 ? top.reduce((sum, item) => sum + Math.abs(item.deltaPct), 0) / top.length : 0;
  ...
  if (drift.isStale) {
    return {
      ...
      factors: [
        {
          key: "high_drift",
          weight: Math.max(0.7, drift.score / 100),
          detail: String(drift.score),
        },
      ],
      correlationId: makeCorrelationId(),
    };
  ...
  const recent = selectedPlan.actions
    ...
    .filter(
      (
        ...
        feedback: NonNullable<ReturnType<typeof getLatestFeedback>>;
      } => Boolean(item.feedback)
    )
    .sort(
      (a, b) => new Date(b.feedback.updatedAt).getTime() - new Date(a.feedback.updatedAt).getTime()
    )
    .slice(0, 4);
...
export function buildLeakPolicy(
  ...
  return {
    ...
    selectedMode:
      getSelectedPlan(plans, snapshot)?.mode ||
      getSnapshotMode(snapshot, "selectedPlanMode") ||
      null,
