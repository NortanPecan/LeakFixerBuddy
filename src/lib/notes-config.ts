...
export const NOTE_TYPES = [
  ...
  { id: "content", label: "Контент", icon: "📚", description: "Конспект книги, статьи, подкаста" },
  {
    ...
    label: "Рефрейминг",
    icon: "🔄",
    description: "Смена мышления: мысль → новый взгляд → действие",
  },
] as const;
...
export function getReframePreview(
  data: ReframeData,
  maxLength = 60
): { oldThought: string; newView: string } {
  return {
    oldThought:
      data.oldThought.length > maxLength
        ? data.oldThought.substring(0, maxLength) + "..."
        : data.oldThought,
    newView:
      data.newView.length > maxLength ? data.newView.substring(0, maxLength) + "..." : data.newView,
  };
}
