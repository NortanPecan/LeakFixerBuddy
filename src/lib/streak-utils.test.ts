...
describe("streak-utils", () => {
  it("keeps the current streak alive when today is still incomplete", () => {
    const result = calculateRitualStreak({
      completions: [entry("2026-04-11"), entry("2026-04-10")],
  ...
  it("ignores gaps on unscheduled days for ritual streaks", () => {
    const result = calculateRitualStreak({
      completions: [entry("2026-04-10"), entry("2026-04-08"), entry("2026-04-06")],
  ...
  it("returns the next scheduled date using the normalized schedule", () => {
    expect(
      formatDateKey(getNextScheduledDate(new Date("2026-04-14T10:00:00.000Z"), [1, 3, 5]))
    ).toBe("2026-04-15");
    expect(formatDateKey(getNextScheduledDate(new Date("2026-04-14T10:00:00.000Z"), []))).toBe(
      "2026-04-15"
    );
  });
});
