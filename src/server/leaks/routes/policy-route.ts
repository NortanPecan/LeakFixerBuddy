...
function shouldAppendSuggestedEvent(
  ...
  const latestAt =
    typeof latestSuggested.at === "string" ? new Date(latestSuggested.at).getTime() : 0;
  const now = Date.now();
  const isFresh = now - latestAt < 60_000;
  const latestCorrelationId =
    typeof latestSuggested.policyCorrelationId === "string"
      ? latestSuggested.policyCorrelationId
      : null;
...
function summarizeCurrentFunnel(snapshot: Record<string, unknown>, correlationId: string | null) {
  if (!correlationId) {
    return null;
  }
  const journal = Array.isArray(snapshot.runJournal) ? snapshot.runJournal : [];
  ...
  const pendingOutcomeActionAgesMinutes = pendingOutcomeActionIds
    ...
    .filter(
      (item): item is { actionId: string; age: number; title: string | null } =>
  ...
  const maxPendingOutcomeAgeMinutes =
    pendingOutcomeActionAgesMinutes.length > 0
      ...
      : null;
  const oldestPendingOutcome =
    pendingOutcomeActionAgesMinutes.sort((a, b) => b.age - a.age)[0] || null;
  ...
  const stuckSignals = {
    noDecision: Boolean(
      suggestedAt && !acceptedAt && !rejectedAt && (suggestedAgeMinutes || 0) >= 120
    ),
    noEntityAfterAccept: Boolean(
      acceptedAt && entityCreatedCount === 0 && (acceptedAgeMinutes || 0) >= 90
    ),
  ...
  const urgency = stuckScore >= 60 ? "high" : stuckScore >= 30 ? "medium" : "low";
  const urgencyReason =
    urgency === "high"
  ...
  const recommendedNudge = stuckSignals.noDecision
    ? "accept_or_reject"
    : stuckSignals.noEntityAfterAccept
      ? "create_entity"
      : stuckSignals.pendingFeedback || stuckSignals.noOutcomeAfterCreate
...
function summarizeLearningSignals(snapshot: Record<string, unknown>) {
  ...
  const attemptByAction = new Map<
    string,
    { actionId: string; actionTitle: string; attempt: number }
  >();
  ...
  actionFeedback.forEach((item) => {
    ...
    if (
      updatedAt &&
      (!stats.lastUpdatedAt ||
        new Date(updatedAt).getTime() > new Date(stats.lastUpdatedAt).getTime())
    ) {
  ...
  const health = Math.max(
    ...
    Math.min(
      100,
      Math.round(
        workedShare * 70 + (1 - failureShare) * 25 + (1 - partialShare) * 5 - repeatPenalty
      )
    )
  );
  const dominantFailureBucket =
    Object.entries(failureBuckets).sort((a, b) => b[1] - a[1])[0]?.[0] || "unknown";
