...
function resolveClusterConfidence(params: {
  ...
  const value = Math.max(
    0.05,
    Math.min(1, baseConfidence + evidenceBoost + sizeBoost - conflictPenalty)
  );
...
export const GET = apiHandler(
  async ({ request }) => {
    ...
    enrichedPatterns.forEach((pattern, index) => {
      ...
      if (match) {
        ...
        match.confidence = Math.max(
          match.confidence,
          clusterMatchConfidence(match.key, pattern.patternKey)
        );
        return;
      }
      clusters.push({
    ...
    return { success: true, patterns: patternsWithCluster, clusters: clustersView };
  },
