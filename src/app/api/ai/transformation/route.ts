...
export async function GET(request: NextRequest) {
  ...
  if (cached) {
    return NextResponse.json({ narrative: cached.response, cached: true, createdAt: cached.createdAt })
  }
  ...
  const [
    ...
    db.gymWorkout.count({ where: { period: { userId }, date: { gte: createdAt, lte: first30End }, status: 'completed' } }),
    db.gymWorkout.count({ where: { period: { userId }, date: { gte: last30Start }, status: 'completed' } }),
    db.ritualCompletion.count({ where: { userId, date: { gte: createdAt, lte: first30End }, completed: true } }),
  ...
  const avgEnergyFirst = avg(first30States.map(d => d.energy).filter((v): v is number => v !== null))
  const avgEnergyLast = avg(last30States.map(d => d.energy).filter((v): v is number => v !== null))
  const avgCalFirst = avg(first30Fitness.map(d => d.calories).filter((v): v is number => v !== null))
  const avgCalLast = avg(last30Fitness.map(d => d.calories).filter((v): v is number => v !== null))
  const ritualRateFirst = activeRituals > 0 ? Math.round((first30Rituals / (activeRituals * 30)) * 100) : null
  const ritualRateLast = activeRituals > 0 ? Math.round((last30Rituals / (activeRituals * 30)) * 100) : null
  const weightDelta =
  ...
  try {
    const result = await callAI(TRANSFORMATION_SYSTEM, lines, { userId, callType: 'transformation' })
