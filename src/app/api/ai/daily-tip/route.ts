...
async function buildUserContext(userId: string): Promise<string> {
  ...
  const [profile, userProfile, fitnessDays, dailyStates, ritualCompletions, activeRituals, gymWorkouts] =
    await Promise.all([
      ...
      db.ritualCompletion.findMany({ where: { userId, date: { gte: sevenDaysAgo }, completed: true } }),
      db.ritual.count({ where: { userId, status: 'active' } }),
      db.gymWorkout.findMany({ where: { userId, date: { gte: sevenDaysAgo }, status: 'completed' } }),
    ])

  const avgMood = dailyStates.length
    ? Math.round(dailyStates.reduce((s, d) => s + (d.mood ?? 5), 0) / dailyStates.length * 10) / 10
    : null
  const avgEnergy = dailyStates.length
    ? Math.round(dailyStates.reduce((s, d) => s + (d.energy ?? 5), 0) / dailyStates.length * 10) / 10
    : null
  ...
  const lines = [
    ...
    userProfile?.targetWeight ? `Цель по весу: ${userProfile.targetWeight} кг (сейчас ${userProfile.weight ?? '?'} кг)` : null,
  ].filter(Boolean).join('\n')
