...
export async function POST(request: NextRequest) {
  ...
  const rl = await checkRateLimit(request, RATE_LIMITS.AI, "ai:analyze");
...
export async function PATCH(request: NextRequest) {
  ...
  const tried = (pattern.triedSolutions ?? []) as {
    text: string;
    triedAt?: string;
    worked: boolean | null;
  }[];
  const updatedTried = tried.map((s) => (s.text === solutionText ? { ...s, worked } : s));
  ...
  const updatedWhatWorked =
    worked && !whatWorked.includes(solutionText) ? [...whatWorked, solutionText] : whatWorked;
