...
async function getRitualsSummary(
  userId: string,
  today: Date
): Promise<{ text: string; keyboard: InlineKeyboard }> {
...
async function getWeightSummary(
  userId: string
): Promise<{ text: string; keyboard: InlineKeyboard }> {
...
async function getMoodEnergySummary(
  userId: string,
  today: Date
): Promise<{ text: string; keyboard: InlineKeyboard }> {
...
async function getLeaksSummary(
  userId: string
): Promise<{ text: string; keyboard: InlineKeyboard }> {
...
async function classifyUnknownInput(
  text: string,
  userId: string
...
function parseNewExercise(text: string): {
  name: string;
        ...
        switch (pending.action) {
          case "sleep":
            ...
            await db.dailyState.upsert({
              ...
              create: { userId: user.id, date: today, mood: 5, energy: 5, sleepHours: num },
            });
          ...
          case "weight":
            ...
            await db.measurement.create({
              data: { userId: user.id, type: "weight", value: num, unit: "kg" },
            });
          ...
          case "mood": {
            ...
            await db.dailyState.upsert({
              where: { userId_date: { userId: user.id, date: today } },
              update: { mood: score },
              create: { userId: user.id, date: today, mood: score, energy: 5 },
            });
          ...
          case "energy": {
            ...
            await db.dailyState.upsert({
              where: { userId_date: { userId: user.id, date: today } },
              update: { energy: score },
              create: { userId: user.id, date: today, mood: 5, energy: score },
            });
