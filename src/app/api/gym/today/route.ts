...
export async function GET(request: NextRequest) {
  try {
    ...
    let nextWorkout: Prisma.GymWorkoutGetPayload<{ include: { workoutTemplate: true } }> | null =
      null;
    if (!todayWorkout) {
    ...
    const lastCycleNotes: Record<string, string> = {};
    ...
    const workout = await db.gymWorkout.update({
      ...
      data: {
        ...
        additionalActivities: additionalActivities
          ? JSON.stringify(additionalActivities)
          : undefined,
...
async function transferWeightsToNewCycle(
  periodId: string,
  ...
  for (const prevWorkout of previousWorkouts) {
    ...
    for (const prevEx of prevWorkout.exercises) {
      ...
      const newEx = newWorkout.exercises.find(
        (e) =>
          (e.workoutTemplateExerciseId &&
            e.workoutTemplateExerciseId === prevEx.workoutTemplateExerciseId) ||
          e.name === prevEx.name
      );
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (
      updateTemplate &&
      exercise.templateId &&
      (weight !== undefined || nextWeight !== undefined)
    ) {
      await db.gymExerciseTemplate.update({
