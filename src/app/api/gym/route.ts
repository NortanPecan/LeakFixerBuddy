...
// v1.3: Interface for wizard exercises
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      ...
      workoutExercises, // v1.3: exercises per workout type { workoutNum: [exercises] }
    } = body;
    ...
    // v1.3: Create GymWorkoutTemplate for each workout day
    const workoutTemplates: Map<number, { id: string; name: string }> = new Map();

    // Get unique workout numbers from daySchedule
    const workoutNums = [
      ...new Set(
        daySchedule
          .filter((d) => d.type === "workout" && d.workoutNum)
          .map((d) => d.workoutNum as number)
      ),
    ];

    for (const workoutNum of workoutNums) {
      const dayItem = daySchedule.find((d) => d.type === "workout" && d.workoutNum === workoutNum);
      const template = await db.gymWorkoutTemplate.create({
        data: {
          periodId: period.id,
          workoutNum,
          name: dayItem?.name || `Тренировка ${workoutNum}`,
          muscleGroups: JSON.stringify(dayItem?.muscleGroups || []),
        },
      });
      workoutTemplates.set(workoutNum, { id: template.id, name: template.name });

      // v1.3: Create GymWorkoutTemplateExercise for each exercise
      const exercises = (workoutExercises?.[workoutNum] || []) as WizardExercise[];
      for (const ex of exercises) {
        // Get exercise template data if available
        let defaultScheme: string | null = null;
        let defaultWeight: number | null = null;

        if (ex.templateId) {
          const exerciseTemplate = await db.gymExerciseTemplate.findUnique({
            where: { id: ex.templateId },
          });
          if (exerciseTemplate) {
            defaultScheme = exerciseTemplate.defaultScheme;
            defaultWeight = exerciseTemplate.currentWeight || exerciseTemplate.nextWeight;
          }
        }

        await db.gymWorkoutTemplateExercise.create({
          data: {
            workoutTemplateId: template.id,
            exerciseTemplateId: ex.templateId || null,
            name: ex.name,
    ...
    for (let cycle = 1; cycle <= totalCyc; cycle++) {
      ...
      for (const dayItem of daySchedule) {
        if (dayItem.type === "workout" && dayItem.workoutNum) {
          ...
          const template = workoutTemplates.get(dayItem.workoutNum);
    ...
    // v1.3: Generate GymExercise for each workout from templates
    // Get all workouts with their template IDs
    const createdWorkouts = await db.gymWorkout.findMany({
      where: { periodId: period.id },
      orderBy: { date: "asc" },
    });

    // Get all template exercises
    const templateExercises = await db.gymWorkoutTemplateExercise.findMany({
      where: {
        workoutTemplateId: { in: Array.from(workoutTemplates.values()).map((t) => t.id) },
      },
      include: {
    ...
    // Group template exercises by workoutTemplateId
    const templateExercisesByTemplate = new Map<string, typeof templateExercises>();
    for (const te of templateExercises) {
      const existing = templateExercisesByTemplate.get(te.workoutTemplateId) || [];
      existing.push(te);
      templateExercisesByTemplate.set(te.workoutTemplateId, existing);
    }
    ...
    for (const workout of createdWorkouts) {
      if (!workout.workoutTemplateId) continue;

      const templateExs = templateExercisesByTemplate.get(workout.workoutTemplateId) || [];
      for (const te of templateExs) {
        exercisesToCreate.push({
          ...
          order: te.order,
          templateId: te.exerciseTemplateId,
          workoutTemplateExerciseId: te.id,
          repsScheme: te.defaultScheme || te.exerciseTemplate?.defaultScheme || null,
          weight: te.defaultWeight || te.exerciseTemplate?.currentWeight || null,
        });
      }
    }

    if (exercisesToCreate.length > 0) {
      await db.gymExercise.createMany({ data: exercisesToCreate });
    }
...
function generateDaySchedule(
  ...
  for (let dayNum = 1; dayNum <= cycleLen; dayNum++) {
    ...
    const crossesWorkout =
      Math.floor(position) !== Math.floor(nextPosition) ||
