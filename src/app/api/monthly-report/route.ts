...
function buildWeekSummary(weekNum: number, days: DayData[]): WeekSummary {
  ...
  const ritualsRate = daysWithRituals.length > 0
    ? avg(daysWithRituals.map(d => d.ritualsTotal > 0 ? d.ritualsCompleted / d.ritualsTotal : 0)) * 100
    : 0
...
function buildMonthlySummary(days: DayData[]): MonthlySummary {
  ...
  return {
    ...
    ritualsCompletionRate: daysWithRituals.length > 0
      ? Math.round(avg(daysWithRituals.map(d => d.ritualsTotal > 0 ? d.ritualsCompleted / d.ritualsTotal : 0)) * 100)
      : 0,
...
function detectMonthlyLeaks(days: DayData[], weeks: WeekSummary[]): MonthlyLeak[] {
  ...
  if (ritualRates.length >= 3) {
    ...
    if (firstHalf - secondHalf > 20) {
      leaks.push({
        ...
        recommendation: 'Пересмотри список ритуалов — возможно, он слишком большой или нереалистичный.',
      })
    }
  }
  ...
  if (stressDays.length >= 5) {
    leaks.push({
      ...
      recommendation: 'Введи практику снятия стресса: дыхание, прогулка, медитация. Минимум 10 мин/день.',
    })
  }
  ...
  if (pastDays >= 14 && totalCheckins / pastDays < 0.4) {
    leaks.push({
      ...
      recommendation: 'Трекинг — основа улучшений. Начни с одного вопроса утром: "Сколько энергии сегодня?"',
    })
  }
...
function detectTrends(weeks: WeekSummary[]): Trend[] {
  ...
  const addTrend = (metric: keyof WeekSummary, label: string, emoji: string) => {
    ...
    if (v1 === 0 && v2 === 0) return
    const delta = v2 - v1
    const direction: 'up' | 'down' | 'stable' = Math.abs(delta) < 0.5 ? 'stable' : delta > 0 ? 'up' : 'down'
    trends.push({ metric: metric as string, direction, delta: Math.round(delta * 10) / 10, label, emoji })
  }
  ...
  if (first.avgEveningRating > 0 || last.avgEveningRating > 0) addTrend('avgEveningRating', 'Оценка дня', '🌙')
