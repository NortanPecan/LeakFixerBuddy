...
import { formatDateKey, getStartOfDay, getStartOfNextDay } from "@/lib/date-utils";
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (let index = 0; index < 7; index += 1) {
      ...
      const completedForDate = weeklyLogs.filter(
        (log) => formatDateKey(log.date) === dateKey
      ).length;
    ...
    const habitsWithStats = await Promise.all(
      habits.map(async (habit) => {
        ...
        const last7Days = Array.from({ length: 7 }, (_, index) => {
          ...
          const completed =
            frequency === "weekly" ? (log?.count ?? 0) > 0 : (log?.count ?? 0) >= targetCount;
