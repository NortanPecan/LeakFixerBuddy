...
export async function GET(request: NextRequest) {
  try {
    ...
    const tasksResult = await journeyTasks().select("*").eq("progress_id", progress.id).get();

    // Get unlocks for this progress
    const unlocksResult = await journeyUnlocks().select("*").eq("progress_id", progress.id).get();
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingResult.data && existingResult.data.goal) {
      ...
      return NextResponse.json(
        { error: "Journey already started", progress: existingResult.data },
        { status: 400 }
      );
    }
    ...
    console.log("[Journey POST] Create result:", {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (goal) {
      const updateResult = await journeyProgress().eq("user_id", user.id).update({ goal });
    ...
    if (completeDay) {
      ...
      const updatedResult = await journeyProgress().eq("id", progress.id).update(updateData);
