...
function calcTDEE(weight: number, height: number, age: number, sex: string, workProfile?: string | null): number {
  ...
  const multiplier = workProfile === 'physical' ? 1.725
    : workProfile === 'mixed' ? 1.55
    : workProfile === 'variable' ? 1.375
...
export async function GET(request: NextRequest) {
  ...
  if (hasGoal) {
    ...
    const totalDays = Math.max(1, Math.round((deadline.getTime() - new Date(startDate).getTime()) / 86400000))
    ...
    const dailyTarget = tdee ? Math.round(tdee - adaptiveDailyDeficit) : (profile.targetCalories ?? null)
    ...
    const weekData = Array.from({ length: 7 }, (_, i) => {
      ...
      d.setHours(0, 0, 0, 0)
      const dayRecord = weekDays.find(wd => new Date(wd.date).toDateString() === d.toDateString())
      return {
    ...
    const avgDailyCalories = weekCalories / Math.max(1, weekData.filter(d => d.calories !== null).length)
  ...
  try {
    ...
    if (clearGoal) {
      await db.userProfile.upsert({
        where: { userId },
        create: { userId, targetWeight: null, weightDeadline: null, weightStart: null, weightStartAt: null },
        update: { targetWeight: null, weightDeadline: null, weightStart: null, weightStartAt: null },
      })
