...
export async function GET(request: NextRequest) {
  try {
    ...
    if (weightStart && targetWeight && currentWeight) {
      ...
      progressPercent =
        totalToLose !== 0 ? Math.min(100, Math.max(0, (lost / totalToLose) * 100)) : 0;
    }
    ...
    let forecast: {
      ...
      deadlineStatus: { willMakeIt: boolean; daysDifference: number; message: string } | null;
    } | null = null;
      ...
      if (weeksPassed > 0) {
        ...
        if (weightDeadline && predictedDate) {
          ...
          const daysDiff = Math.round(
            (deadlineDate.getTime() - predictedDate.getTime()) / (1000 * 60 * 60 * 24)
          );
          deadlineStatus = {
