...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const summary = {
      ...
      avgEveningRating: avg(
        days.map((d) => d.eveningRating).filter((v): v is number => v !== null)
      ),
...
function detectLeaks(days: DayData[]): LeakHint[] {
  ...
  if (daysWithRituals.length >= 3) {
    const lowRitualDays = daysWithRituals.filter(
      (d) => d.ritualsTotal > 0 && d.ritualsCompleted / d.ritualsTotal < 0.5
    );
  ...
  if (totalHabitLogs === 0 && days.some((d) => new Date(d.date) < new Date())) {
    hints.push({
      ...
      emoji: "🔄",
      message:
        "Привычки на этой неделе не отмечались. Трекинг привычек помогает выявлять паттерны.",
    });
  }

  // Hint 9: Rituals → day quality correlation
  const daysWithRitualsAndRating = days.filter(
    (d) => d.ritualsTotal > 0 && d.eveningRating !== null
  );
  if (daysWithRitualsAndRating.length >= 3) {
    const highRitualDays = daysWithRitualsAndRating.filter(
      (d) => d.ritualsTotal > 0 && d.ritualsCompleted / d.ritualsTotal >= 0.8
    );
    const lowRitualDays2 = daysWithRitualsAndRating.filter(
      (d) => d.ritualsTotal > 0 && d.ritualsCompleted / d.ritualsTotal < 0.5
    );
  ...
  if (weekdayRitualDays.length >= 2 && weekendRitualDays.length >= 1) {
    const weekdayRate = avg(
      weekdayRitualDays.map((d) => (d.ritualsTotal > 0 ? d.ritualsCompleted / d.ritualsTotal : 0))
    );
    const weekendRate = avg(
      weekendRitualDays.map((d) => (d.ritualsTotal > 0 ? d.ritualsCompleted / d.ritualsTotal : 0))
    );
