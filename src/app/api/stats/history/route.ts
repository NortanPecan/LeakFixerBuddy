  ...
  try {
    ...
    const [
      ...
      db.dailyCheckin
        ...
        .catch(
          () =>
            [] as { date: Date; type: string; energy: number | null; dayRating: number | null }[]
        ),
    ...
    sortedDates.forEach(([_, entry]) => {
      const hasActivity =
        entry.ritualsCompleted > 0 || entry.habitsCompleted > 0 || entry.tasksCompleted > 0;
      if (hasActivity) {
        tempStreak++;
    ...
    const history = sortedDates.map(([_, entry]) => ({
      ...
      habitsRate:
        entry.habitsTotal > 0 ? Math.round((entry.habitsCompleted / entry.habitsTotal) * 100) : 0,
...
function getWeeklySummary(
  history: Array<{ date: string; overallScore: number; ritualsRate: number; habitsRate: number }>
) {
  const weeks: Array<{
    week: string;
    avgScore: number;
