...
import { calculateWellbeingScore, countAnsweredQuestions } from "@/lib/wellbeing-utils";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const answers = dailyWellbeing?.answers ? JSON.parse(dailyWellbeing.answers) : {};

    const scores = dailyWellbeing?.scores ? JSON.parse(dailyWellbeing.scores) : null;
