...
export async function GET(request: NextRequest) {
  try {
    ...
    const answers = weeklyRecord?.answers 
      ? JSON.parse(weeklyRecord.answers) 
      : {}
    
    const scores = weeklyRecord?.scores 
      ? JSON.parse(weeklyRecord.scores) 
      : null
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!validPresets.includes(preset)) {
      return NextResponse.json({ error: 'Invalid preset for weekly. Use expanded or full' }, { status: 400 })
    }
