  ...
  return (
    <Card className="bg-card/50 backdrop-blur">
      <CardHeader className="pb-2">
        <CardTitle className="flex items-center gap-2 text-base">🌡️ Как ты сейчас?</CardTitle>
      </CardHeader>
      <CardContent>
        {/* Emotion buttons */}
          ...
          {EMOTIONS.map((e) => (
            <button
              ...
              className={`flex flex-col items-center gap-1 rounded-xl p-2 text-xs transition-all ${
                saving === e.key ? "scale-95 opacity-50" : "hover:bg-muted/60 active:scale-95"
              }`}
            >
        ...
        {recent.length > 0 && (
          <div className="border-border/30 space-y-1 border-t pt-2">
            {recent.map((log) => {
              ...
              const time = new Date(log.createdAt).toLocaleTimeString("ru", {
                hour: "2-digit",
                minute: "2-digit",
              });
              return (
                <div key={log.id} className="flex items-center justify-between text-xs">
                  ...
                  <button
                    ...
                    className="text-muted-foreground/30 hover:text-destructive px-1 text-xs"
                  >
                    ×
                  </button>
                </div>
