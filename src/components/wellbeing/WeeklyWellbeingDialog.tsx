...
import { getISOWeekDates, countAnsweredQuestions, getWeeklyQuestionsCount } from '@/lib/wellbeing-utils'
...
export function WeeklyWellbeingDialog({
  ...
  useEffect(() => {
    if (open && preset !== 'core') {
      ...
      setQuestions(weeklyQuestions.map(q => ({
        ...q,
  ...
  const handleAnswerChange = (questionId: string, value: number) => {
    ...
    setQuestions(prev => prev.map(q => 
      q.id === questionId ? { ...q, currentValue: value } : q
    ))
  }
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="max-w-md sm:max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Неделя {week} ({weekDates.start.toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' })} — {weekDates.end.toLocaleDateString('ru-RU', { day: 'numeric', month: 'short' })})
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-1">
            ...
            <div className="text-xs text-muted-foreground text-right">
              ...
              {answeredCount < totalQuestions && answeredCount > 0 && (
                <span className="text-amber-500 ml-2">(частично)</span>
              )}
            </div>
          </div>
