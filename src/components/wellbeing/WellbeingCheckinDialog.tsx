...
import { ChevronLeft, ChevronRight, Check, Loader2, Clock, AlertCircle } from "lucide-react";
    ...
    if (currentQuestion.scale === "hours") {
      return (
        <div className="space-y-4">
          <div className="text-center">
            {currentValue !== undefined ? (
              <>
                ...
                <span className="text-muted-foreground ml-2 text-xl">часов</span>
              </>
            ) : (
              <span className="text-muted-foreground text-xl">Выберите значение</span>
            )}
          </div>
          ...
          <div className="flex justify-center gap-2">
            {[6, 7, 8, 9].map((val) => (
              <Button
                ...
                size="sm"
                className={`h-10 w-12 ${currentValue === val ? "bg-primary text-primary-foreground" : ""}`}
          ...
          <div className="text-muted-foreground flex justify-between px-4 text-xs">
            <span>0ч</span>
            <span>12ч+</span>
          </div>
        </div>
        ...
        <div className="flex justify-center gap-3">
          {[1, 2, 3, 4, 5].map((val) => (
            <Button
              ...
              variant={currentValue === val ? "default" : "outline"}
              size="lg"
              className={`h-14 w-14 rounded-full text-lg font-bold transition-all ${
                currentValue === val
      ...
      <Dialog open={open} onOpenChange={handleOpenChange}>
        <DialogContent className="max-w-md sm:max-w-lg">
          ...
          <div className="py-4">
            ...
            <h3 className="mb-6 text-lg font-medium">{currentQuestion.question}</h3>
            {renderSlider()}
          </div>
          ...
          <div className="flex gap-2">
            ...
            {currentIndex < questions.length - 1 && !hasAnsweredCurrent && (
              <Button variant="ghost" className="text-muted-foreground flex-1" disabled>
                Выберите ответ
              </Button>
            )}
          </div>
