...
export function WeightRecordsModal({ open, onOpenChange }: WeightRecordsModalProps) {
  ...
  const loadData = async (reset = false) => {
    ...
    try {
      ...
      const res = await fetch(`/api/weight/records?userId=${user.id}&skip=${skip}&take=${PAGE_SIZE}`)
                  ...
                  >
                    <div className="text-left">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Средний: {day.avg.toFixed(1)} кг · {day.count} {day.count === 1 ? 'запись' : day.count < 5 ? 'записи' : 'записей'}
                      </p>
                    </div>
                      ...
                      {day.measurements.map(m => (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-muted-foreground">{formatTime(m.date)}</span>
                          </div>
