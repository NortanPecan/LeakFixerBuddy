    ...
    <div className="flex flex-col gap-4 pb-20">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">Создай полезную привычку</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Input
            placeholder="Например: Медитация утром"
            value={title}
          ...
          <div className="grid grid-cols-3 gap-2">
            {(
              ...
              <button
                key={key}
                className={
                  "rounded-xl p-2 text-sm " + (category === key ? value.color : "bg-muted/30")
                }
            ...
            {DAY_PRESETS.filter((p) => p.value !== "custom").map((preset) => (
              <button
                ...
                className={
                  ...
                  (dayPreset === preset.value ? "bg-primary/20 text-primary" : "bg-muted/30")
                }
              ...
              {DAYS_OF_WEEK.map((day) => (
                <button
                  ...
                  className={
                    ...
                    (customDays.includes(day.value) ? "bg-primary text-white" : "bg-muted/30")
                  }
          ...
          <div className="grid grid-cols-4 gap-2">
            {(Object.entries(TIME_WINDOW_LABELS) as [TimeWindow, string][]).map(([key, label]) => (
              <button
                ...
                className={
                  "rounded-xl p-2 text-sm " +
                  (timeWindow === key ? "bg-primary/20 text-primary" : "bg-muted/30")
                }
          ...
          <div className="flex gap-2">
            {(
              ...
              <button
                key={key}
                className={
                  "flex-1 rounded-xl p-3 text-center text-sm " +
                  (selectedAttributes.includes(key) ? value.color + " text-white" : "bg-muted/30")
                }
        ...
        <div className="space-y-2">
          <Label>Цель</Label>
          <Input
            placeholder="Зачем тебе этот ритуал?"
            value={goalShort}
            onChange={(e) => setGoalShort(e.target.value)}
          />
        </div>
      ...
      <div className="flex gap-2 pt-4">
        <Button variant="outline" className="flex-1" onClick={() => setScreen("rituals")}>
          Отмена
        </Button>
        <Button
