  ...
  return (
    <div className="flex flex-col gap-4 pb-20">
      ...
      <div className="space-y-0">
          ...
          return (
            <div key={task.id} className="flex gap-3">
              ...
              <div className="flex flex-col items-center">
                ...
                {/* Connecting line */}
                {!isLast && (
                  <div className={`h-12 w-0.5 ${isDone ? "bg-emerald-500" : "bg-muted"}`} />
                )}
              </div>
        ...
        {chain.status !== "completed" && (
          <div className="flex gap-3">
            <div className="flex flex-col items-center">
              <button
                className="bg-muted border-muted-foreground/30 hover:border-primary flex h-8 w-8 shrink-0 items-center justify-center rounded-full border-2 border-dashed transition-colors"
      ...
      {chain.status !== "completed" && todoTasks.length === 0 && (
        <Card className="border-emerald-500/20 bg-gradient-to-r from-emerald-500/10 to-emerald-500/5">
          <CardContent className="pt-4">
            <div className="flex items-center gap-3">
              <Sparkles className="h-5 w-5 text-emerald-400" />
              <div className="flex-1">
                <p className="font-medium">Цель достигнута?</p>
                <p className="text-muted-foreground text-sm">Завершить цепочку</p>
              </div>
              <Button
                size="sm"
        ...
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1" onClick={() => setShowAddStep(false)}>
                Отмена
              </Button>
              <Button
      ...
      <Dialog open={showCompleteModal} onOpenChange={setShowCompleteModal}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 pt-4">
            <p className="text-muted-foreground text-sm">Отлично! Что дальше?</p>
