  ...
  return (
    <div className="flex flex-col gap-4 pb-20">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-foreground text-2xl font-bold">Навыки</h1>
          <p className="text-muted-foreground text-sm">
            {!isLoading && `${skills.length} навыков`}
          </p>
        </div>
        <Button size="sm" onClick={() => setShowDialog(true)}>
          <Plus className="mr-1 h-4 w-4" /> Новый
        </Button>
      </div>
        ...
        <Card className="border-dashed">
          <CardContent className="pt-6 text-center">
            ...
            <Button onClick={() => setShowDialog(true)}>
              <Plus className="mr-2 h-4 w-4" /> Добавить навык
            </Button>
          </CardContent>
        </Card>
      )}
          ...
          {skills.map((skill) => (
            <Card key={skill.id} className="bg-card/50 backdrop-blur">
              <CardContent className="pt-4">
                ...
                <Progress value={(skill.xp / skill.xpToNext) * 100} className="mb-2 h-2" />
                <div className="flex items-center justify-between">
