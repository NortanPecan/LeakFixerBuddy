...
import { ArrowLeft, Target, Flame, TrendingDown, TrendingUp, AlertTriangle, CheckCircle2, Edit3 } from 'lucide-react'
...
export function CalorieGoalScreen() {
  ...
  const handleSave = async () => {
    ...
    if (isNaN(tw) || tw <= 0) { showErrorToast('Некорректный вес'); return }
    if (new Date(formDeadline) <= new Date()) { showErrorToast('Дедлайн должен быть в будущем'); return }
    ...
    <div className="flex flex-col gap-4 pb-24">
      ...
      <div className="flex items-center gap-3 pt-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-muted-foreground">Адаптивный план питания</p>
        </div>
        {goal && (
      ...
      {!loading && (showForm || !goal) && (
        <Card className="border-primary/20">
          ...
          <CardContent className="space-y-4">
            ...
            {/* Preview calculation */}
            {formTargetWeight && formDeadline && profile?.weight && profile?.tdee && (() => {
              const tw = parseFloat(formTargetWeight)
              ...
              return (
                <div className={`p-3 rounded-lg border text-sm space-y-1 ${isAgg ? 'bg-red-500/10 border-red-500/20' : 'bg-emerald-500/10 border-emerald-500/20'}`}>
                  <div className="font-medium">{isAgg ? '⚠️ Слишком агрессивно' : '✅ Реалистичный план'}</div>
                  <div className="text-muted-foreground">TDEE: {profile.tdee} ккал · Дефицит: {Math.round(deficit)} ккал/день</div>
                  <div className="font-semibold text-lg">{daily > 0 ? daily : '—'} ккал/день</div>
                  {isAgg && <div className="text-red-400 text-xs">Максимальный безопасный дефицит — 1000 ккал/день. Попробуй сдвинуть дедлайн.</div>}
                </div>
              )
            })()}
        ...
        <>
            ...
            <Card className="border-red-500/30 bg-red-500/10">
              <CardContent className="p-4 flex items-start gap-3">
                ...
                <div className="text-sm">
                  ...
                  <div className="text-muted-foreground">Требуемый дефицит {goal.adaptiveDailyDeficit} ккал/день — более 1 кг в неделю. Рекомендуем скорректировать цель.</div>
                </div>
              </CardContent>
            </Card>
          )}
            ...
            <Card className="border-amber-500/30 bg-amber-500/10">
              <CardContent className="p-4 flex items-start gap-3">
                ...
                <div className="text-sm">
                  ...
                  <div className="text-muted-foreground">Дефицит {goal.adaptiveDailyDeficit} ккал/день. Следи за самочувствием.</div>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          <Card>
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>Сейчас: <strong className="text-foreground">{goal.currentWeight} кг</strong></span>
          ...
          <Card className="bg-gradient-to-br from-primary/10 to-primary/5 border-primary/20">
            <CardContent className="p-4">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <div className="text-sm text-muted-foreground">Дневная цель (адаптивная)</div>
                  <div className="text-3xl font-bold">{goal.dailyTarget ?? '—'} <span className="text-base font-normal text-muted-foreground">ккал</span></div>
                  {goal.tdee && <div className="text-xs text-muted-foreground">TDEE {goal.tdee} − дефицит {goal.adaptiveDailyDeficit}</div>}
                </div>
              </div>
              ...
              {todayCalories !== null && todayTarget && (
                <div className="mt-3 space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className={todayCalories > todayTarget ? 'text-red-400' : 'text-emerald-400'}>
                  ...
                  {todayRemaining !== null && (
                    <div className="text-xs text-center">
                      {todayRemaining > 0
                        ...
                        : <span className="text-red-400">⚠ Превышение на {Math.abs(todayRemaining)} ккал</span>
                      }
                    </div>
                  )}
                </div>
          ...
          <Card>
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                ...
                <Badge
                  variant="outline"
                  className={goal.weekSurplus > 0 ? 'border-red-500/40 text-red-400' : 'border-emerald-500/40 text-emerald-400'}
                >
                  {goal.weekSurplus > 0
                    ? <><TrendingUp className="w-3 h-3 mr-1" />+{goal.weekSurplus} ккал</>
                    : <><TrendingDown className="w-3 h-3 mr-1" />{goal.weekSurplus} ккал</>
                  }
                </Badge>
              </div>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="grid grid-cols-7 gap-1">
                {goal.weekData.map((day, i) => {
                  const pct = day.calories !== null && day.target ? Math.min(120, (day.calories / day.target) * 100) : 0
                  const isOver = day.calories !== null && day.target ? day.calories > day.target : false
                  const isToday = day.date === new Date().toISOString().split('T')[0]
                  return (
                    <div key={day.date} className="flex flex-col items-center gap-1">
                      <div className="text-[10px] text-muted-foreground">{weekDayLabels[new Date(day.date).getDay() === 0 ? 6 : new Date(day.date).getDay() - 1]}</div>
                      <div className="relative w-full h-16 bg-muted/30 rounded-md overflow-hidden flex items-end">
                        {day.calories !== null && (
                          <div
                        ...
                        {/* Target line */}
                        <div className="absolute w-full h-px bg-white/30 bottom-[83%]" />
                      </div>
                      <div className={`text-[9px] ${isToday ? 'font-bold text-primary' : 'text-muted-foreground'}`}>
              ...
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>Съедено: <strong className="text-foreground">{goal.weekCalories}</strong> ккал</span>
                <span>Цель: <strong className="text-foreground">{goal.weekTarget}</strong> ккал</span>
              </div>
              ...
              {goal.weekSurplus > 500 && (
                <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-sm">
                  ...
                  <div className="text-muted-foreground">
                    За неделю перебор {goal.weekSurplus} ккал. Завтра цель автоматически скорректирована.
                    Новый дневной таргет: <strong>{goal.dailyTarget}</strong> ккал.
                  </div>
                </div>
              )}
              {goal.weekSurplus < -500 && (
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <div className="text-sm text-muted-foreground">Прогноз к {goal.deadline}</div>
                  <div className="text-2xl font-bold mt-1">
                    {goal.projectedWeight} кг
                    <span className={`text-sm font-normal ml-2 ${goal.projectedWeight <= goal.targetWeight ? 'text-emerald-400' : 'text-amber-400'}`}>
                      {goal.projectedWeight <= goal.targetWeight
                        ? <><CheckCircle2 className="w-4 h-4 inline mr-1" />Цель достижима</>
                        : <>+{Math.round((goal.projectedWeight - goal.targetWeight) * 10) / 10} кг от цели</>
                      }
                    </span>
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">На основе среднего потребления за неделю</div>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          {/* Clear goal */}
          <Button variant="ghost" className="text-muted-foreground text-sm" onClick={handleClear} disabled={saving}>
            Удалить цель
          </Button>
        </>
      )}
