-- Migration: Journey Course Tables for Supabase
-- Run this in Supabase SQL Editor to create the Journey tables
-- Updated with full 30-day course content from JOURNEY_COURSE.md

-- ============================================
-- JOURNEY LESSONS TABLE (Static content)
-- ============================================

CREATE TABLE IF NOT EXISTS journey_lessons (
...
COMMENT ON TABLE journey_lessons IS '30-day journey lessons with tasks and rewards';
COMMENT ON COLUMN journey_lessons.day IS 'Day number (1-30)';
COMMENT ON COLUMN journey_lessons.week IS 'Week number (1-4)';
COMMENT ON COLUMN journey_lessons.week_name IS 'Week name: Пробуждение, Закалка, Восхождение, Мастерство';
COMMENT ON COLUMN journey_lessons.story IS 'Story/narrative for the day';
COMMENT ON COLUMN journey_lessons.tasks IS 'JSON array of tasks for this day';
COMMENT ON COLUMN journey_lessons.unlocks IS 'JSON array of features to unlock';

-- ============================================
-- JOURNEY PROGRESS TABLE (User progress)
-- ============================================
...
COMMENT ON TABLE journey_progress IS 'User progress through 30-day journey';
COMMENT ON COLUMN journey_progress.goal IS 'User selected goal: all, weight, habits, energy';

-- ============================================
-- JOURNEY TASKS TABLE (Task completions)
-- ============================================

CREATE TABLE IF NOT EXISTS journey_tasks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  progress_id UUID NOT NULL REFERENCES journey_progress(id) ON DELETE CASCADE,
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(progress_id, day, task_id)
);

COMMENT ON TABLE journey_tasks IS 'Individual task completion records';

-- ============================================
-- JOURNEY UNLOCKS TABLE (Feature unlocks)
-- ============================================
...
CREATE TABLE IF NOT EXISTS journey_unlocks (
  ...
  UNIQUE(progress_id, feature)
);

COMMENT ON TABLE journey_unlocks IS 'Features unlocked during journey';
...
COMMENT ON TABLE journey_achievements IS 'Achievements earned during journey';

-- ============================================
-- JOURNEY REFLECTIONS TABLE (Daily reflections)
-- ============================================
...
CREATE TABLE IF NOT EXISTS journey_reflections (
  ...
  text TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  UNIQUE(progress_id, day)
);

COMMENT ON TABLE journey_reflections IS 'User daily reflections during journey';

-- ============================================
-- INDEXES
-- ============================================

CREATE INDEX IF NOT EXISTS idx_journey_lessons_day ON journey_lessons(day);
CREATE INDEX IF NOT EXISTS idx_journey_lessons_week ON journey_lessons(week);
CREATE INDEX IF NOT EXISTS idx_journey_progress_user ON journey_progress(user_id);
CREATE INDEX IF NOT EXISTS idx_journey_tasks_progress ON journey_tasks(progress_id);
CREATE INDEX IF NOT EXISTS idx_journey_tasks_day ON journey_tasks(day);
CREATE INDEX IF NOT EXISTS idx_journey_unlocks_progress ON journey_unlocks(progress_id);
CREATE INDEX IF NOT EXISTS idx_journey_achievements_progress ON journey_achievements(progress_id);
...
ALTER TABLE journey_unlocks ENABLE ROW LEVEL SECURITY;
ALTER TABLE journey_achievements ENABLE ROW LEVEL SECURITY;
ALTER TABLE journey_reflections ENABLE ROW LEVEL SECURITY;

-- Journey lessons are readable by everyone
CREATE POLICY "Journey lessons are readable by all" ON journey_lessons
  FOR SELECT USING (true);

-- Users can read their own progress
CREATE POLICY "Users can read own progress" ON journey_progress
...
CREATE POLICY "Users can read own tasks" ON journey_tasks
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_tasks.progress_id 
      AND journey_progress.user_id::text = auth.uid()::text
    )
  );

-- Users can insert their own tasks
CREATE POLICY "Users can insert own tasks" ON journey_tasks
...
CREATE POLICY "Users can update own tasks" ON journey_tasks
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_tasks.progress_id 
...
-- Similar policies for unlocks, achievements, reflections
CREATE POLICY "Users can read own unlocks" ON journey_unlocks
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_unlocks.progress_id 
      AND journey_progress.user_id::text = auth.uid()::text
    )
  );

CREATE POLICY "Users can insert own unlocks" ON journey_unlocks
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_unlocks.progress_id 
...
CREATE POLICY "Users can read own achievements" ON journey_achievements
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_achievements.progress_id 
      AND journey_progress.user_id::text = auth.uid()::text
    )
  );

CREATE POLICY "Users can insert own achievements" ON journey_achievements
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_achievements.progress_id 
...
CREATE POLICY "Users can read own reflections" ON journey_reflections
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_reflections.progress_id 
      AND journey_progress.user_id::text = auth.uid()::text
    )
  );

CREATE POLICY "Users can insert own reflections" ON journey_reflections
  FOR INSERT WITH CHECK (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_reflections.progress_id 
...
CREATE POLICY "Users can update own reflections" ON journey_reflections
  FOR UPDATE USING (
    EXISTS (
      SELECT 1 FROM journey_progress 
      WHERE journey_progress.id = journey_reflections.progress_id 
...
-- Apply triggers
DROP TRIGGER IF EXISTS update_journey_lessons_updated_at ON journey_lessons;
CREATE TRIGGER update_journey_lessons_updated_at
  BEFORE UPDATE ON journey_lessons
